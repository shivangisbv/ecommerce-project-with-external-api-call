...
class Products {
    async getProducts() {
        try {
            let result = await fetch("assert/js/products.json");
...
class UI {
    ...
    addCartItem(item) {
        ...
        <div>
            <h4>${item.title}</h4>
            <h5>$ ${item.price}</h5>
            <span class="remove-item " data-id = ${item.id} >Removed</span>
        </div>
        <div>
...
document.addEventListener("DOMContentLoaded",() => {
    ...
    // get all products
    //  first practice products.getProducts().then(products => console.log(products));
